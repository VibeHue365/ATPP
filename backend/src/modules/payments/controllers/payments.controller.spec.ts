...
describe('PaymentsController PayOS webhook', () => {
  ...
  function createController(claimState: 'claimed' | 'processed' | 'processing' = 'claimed') {
    const paymentsService = { confirmPayment: jest.fn().mockResolvedValue({}) };
    ...
    const webhookEventRepository = {
      claimVerifiedEvent: jest.fn().mockResolvedValue({ state: claimState, event: {} }),
    ...
    return { controller, paymentsService, paymentsRepository, webhookEventRepository };
  }

  it('processes a claimed PAID event exactly once', async () => {
    const fixture = createController('claimed');

    await expect(fixture.controller.handlePayOSWebhook(body as any)).resolves.toEqual({ status: 'success' });

    expect(fixture.webhookEventRepository.claimVerifiedEvent).toHaveBeenCalledWith(
      'payos_123456_PAID',
      body.data,
      body.signature,
    );
    expect(fixture.paymentsService.confirmPayment).toHaveBeenCalledWith('PAY123');
    expect(fixture.webhookEventRepository.markProcessed).toHaveBeenCalledWith('payos_123456_PAID');
  });
  ...
  it('does not apply payment again for an already processed or in-progress event', async () => {
    ...
    await expect(processed.controller.handlePayOSWebhook(body as any)).resolves.toEqual({
      status: 'success', note: 'already_processed',
    });
    await expect(processing.controller.handlePayOSWebhook(body as any)).resolves.toEqual({
      status: 'success', note: 'already_processing',
    });
  ...
  it('records a processing failure so a later webhook delivery can retry', async () => {
    ...
    await expect(fixture.controller.handlePayOSWebhook(body as any)).rejects.toBeInstanceOf(BadRequestException);
